...
- (void)setupScrollPerspective
{
    ...
    //z distance
  float distance = _viewFrame.size.height;//[[UIScreen mainScreen] bounds].size.height;
...
//                                      [[UIScreen mainScreen] bounds].size.height/3);
  self.layer.position = CGPointMake(_viewFrame.size.width/2,
                                    _viewFrame.size.height/3);

}

+ (id)scrollViewWithStartPoint:(CGPoint)startPoint ViewFrame:(CGRect)viewFrame
{
    // nib ファイルから読み込む
...
{
    CGFloat animationWidth  =  _viewFrame.size.width;
    CGFloat animationHeight =  _viewFrame.size.height + 120;
